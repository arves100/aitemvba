...
extern unsigned char gbpPythonSerial(u8);
...
VBA::~VBA()
{
  ...
  if (autoBattle)
  {
	...
	if (pAIScript)
	{
		fclose(pAIScript->m_autoBattlePStream);
		delete pAIScript;
		pAIScript = NULL;
	}
  ...
  if(rewindMemory)
    ...
    /* Link 
------------------------------------*/
	if((jjj=fopen("vbalink.ini", "wb"))!=NULL){
		putc(lanlink.wasactive, jjj);
		fclose(jjj);
	}		
	if(lanlink.active)
		CloseLanLink();
...
void systemShowSpeed(int speed)
{
  ...
  if(theApp.videoOption <= VIDEO_4X && theApp.showSpeed) {
    ...
    if(theApp.showSpeed == 1)
      buffer.Format("AI-TEM VBA-%3d%%", systemSpeed);
    else
      buffer.Format("AI-TEM VBA-%3d%%(%d, %d fps)", systemSpeed,
                    systemFrameSkip,
...
void VBA::loadSettings()
{
  ...
  winGbPytonScriptEnabled = regQueryDwordValue("gbPython", false) ? true : false;

  if (winGbPytonScriptEnabled)
	  gbpSerialFunction = gbpPythonSerial;
  else
	  gbpSerialFunction = NULL;

  pauseWhenInactive = regQueryDwordValue("pauseWhenInactive", false) ?
    true : false;
...
//----------------------------------------------------------------
void
VBA::MyAIreadData(u32 address, int len, u8 *data)
{
  if(emulating && rom != NULL) {
    for(int i = 0; i < len; i++) {
		u8 a = CPUReadByteQuick(address);
...
void
VBA::MyAIwriteData(u32 address, int len, u32 data)
{
    switch(len)
	{
		case 8:
			CPUWriteByte(address, data);
		break;
		case 16:
			CPUWriteHalfWord(address, data);
		break;
		case 32:
...
void
VBA::MyAICallClosePython()
{
	((MainWnd*)m_pMainWnd)->CallOnStopPythonScript();
}
//-----------------------------------------------------------------