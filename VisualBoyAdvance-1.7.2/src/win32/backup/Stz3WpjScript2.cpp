...
Stz3WpjScript::CalCharState()
{
	...
	if (m_p1.m_deltaX > 180) // Far 180-360
	{
		state = state + 12;
	}
	else if (m_p1.m_deltaX > 115) // Middle 115-180
	{
		state = state + 8;
	}
...
Stz3WpjScript::ExecuteStClose0()
{
	...
	if (m_p1Ani <= 732) // 732 is maximum of normal action
	{
		...
		{
			result = Shoryuken(PRESS_B);
		}
		else //if (ran <= 9)
		{
			result = PressButtonNFrame(PRESS_DOWN | PRESS_A, 4);
		}
	}
...
Stz3WpjScript::ExecuteStMiddle2() // Air - Ground
{
	...
	if (m_p1Ani <= 732) // 732 is maximum of normal action
	{
		...
		{
			result = PressButtonNFrame(PRESS_A, 10);
		}
	}
	return result;
}
