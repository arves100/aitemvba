...
u32 DirectInput::readDevice(int which)
{
  ...
  {
	  // Do everything here
	  // This place will do every cpu loop
	  res |= theApp.pAIScript->RunAIScript();
  }
  else if (theApp.pythonPlaying)
  {
	  // Do everything here
	  // This place will do every cpu loop
	  int result = theApp.pPython->RunPythonScript();
	  if (result < 0)
	  {
		  theApp.MyAICallClosePython();
		  AfxMessageBox("Python error!, Please see err.txt for more detail.");
		  res = 0;
	  }
	  else
	  {
  ...
  if (theApp.aniRecording)
  {
	  ...
	  res = theApp.pAIScript->RunAutoAnimationRecord();
  }
  else if (theApp.autoBattle)
  {
	  res = theApp.pAIScript->RunAutoBattle();
  }
