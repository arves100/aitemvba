...
BEGIN_MESSAGE_MAP(MainWnd, CWnd)
  ...
  ON_COMMAND(ID_MYAI_STARTMYAI, OnStartMyAI)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STARTMYAI, OnUpdateStartMyAI)
  ON_COMMAND(ID_MYAI_STOPMYAI, OnStopMyAI)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STOPMYAI, OnUpdateStopMyAI)

  ON_COMMAND(ID_MYAI_STARTPYTHONSCRIPT, OnStartPythonScript)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STARTPYTHONSCRIPT, OnUpdateStartPythonScript)
  ON_COMMAND(ID_MYAI_STOPPYTHONSCRIPT, OnStopPythonScript)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STOPPYTHONSCRIPT, OnUpdateStopPythonScript)

  ON_COMMAND(ID_MYAI_STARTANIRECORD, OnStartAnimationRecord)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STARTANIRECORD, OnUpdateStartAnimationRecord)
  ON_COMMAND(ID_MYAI_STOPANIRECORD, OnStopAnimationRecord)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STOPANIRECORD, OnUpdateStopAnimationRecord)
  
  ON_COMMAND(ID_MYAI_WRITEGMDFILE, OnStartWriteGMDFile)

  ON_COMMAND(ID_MYAI_AUTOBATTLE,   OnStartAutoBattle)
  ON_UPDATE_COMMAND_UI(ID_MYAI_AUTOBATTLE, OnUpdateStartAutoBattle)
  ON_COMMAND(ID_MYAI_STOPAUTOBATTLE, OnStopAutoBattle)
  ON_UPDATE_COMMAND_UI(ID_MYAI_STOPAUTOBATTLE, OnUpdateStopAutoBattle)
  ...
  ON_COMMAND(ID_MYAI_ENABLELINKCABLE, OnOptionsPythonScript)
  ON_UPDATE_COMMAND_UI(ID_MYAI_ENABLELINKCABLE, OnUpdateOptionsPythonScript)
  ...
  ON_UPDATE_COMMAND_UI(ID_FILE_LOADGAME_MOSTRECENT, OnUpdateFileLoadgameMostrecent)
	...
	ON_COMMAND(ID_OPTIONS_LINK, OnOptionsLink)
	//}}AFX_MSG_MAP
...
bool MainWnd::FileRun()
{
  ...
  if(rom != NULL || gbRom != NULL) {
	...
	theApp.memCompareWrite.Cleanup();
  ...
  if (rom)
  {
	  strncpy(theApp.romName, (const char *)&rom[0xa0], 12);
	  theApp.romName[12] = 0;
  }
  else
  {
	  // GB/GBC
	  strncpy(theApp.romName, (const char *)&gbRom[0x134], 10);
	  theApp.romName[10] = 0;
  }
  //-----------------------------------------------
...
void MainWnd::writeBatteryFile()
{
  ...
  if(isDriveRoot(saveDir))
		filename.Format("%s%s.sa%1d", saveDir, buffer, linkid+1);
  else
    filename.Format("%s\\%s.sa%1d", saveDir, buffer, linkid+1);
...
void MainWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
  ...
  bool a = (nState == WA_ACTIVE) || (nState == WA_CLICKACTIVE);
  		...
  		/*if(extTicks!=NULL&&*extTicks>1){
		sprintf(trpaslik, "kjh %d", *extTicks);
	  MessageBox(NULL, trpaslik, "Boordel", MB_OK);
		}*/
  if(a && theApp.input) {
    theApp.active = a;
